  ...
  try {
    ...
    if (res.data.message.token) {
      ...
      NotifyUser(
        ...
        "Sorry, Could not register you at this time."
      );
    }
  ...
  switch (type) {
    case "verify":
      try {
        ...
        resp_result.message = error.message;
        NotifyUser(
          NOTIFICATION_TYPE.ERR,
      ...
      break;
    case "forgot_password": //after the above case this will be automatically called
        try {
            await axios
              .put(
                `/api/users/forgot_password`,
                { user: req_body.user, type: req_body.type },
                headersConfig
              )
            ...
            resp_result.success = false;
            resp_result.message = error.message;
            console.log("Error from [EditUser] with message:", error.message);
